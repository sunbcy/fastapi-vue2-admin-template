...
# 根据歌曲URL下载歌曲
def download_song(url, songname):  # 无法下载VIP
    print(f'正在下载 <{songname}>')
    res = requests.get(url, proxies=check_proxy())
    with open(f'{songname}.mp3', 'wb') as f:
        f.write(res.content)


def download_song_by_id(song_id, songname):  # 也不可以下载VIP歌曲 但是VIP尊享歌曲会找不到源地址
    print(f'正在下载 <{songname}>-<{song_id}>')
    url = f'http://music.163.com/song/media/outer/url?id={song_id}'
    music = requests.get(url, headers=headers, proxies=check_proxy())
    if '很抱歉，你要查找的网页找不到' in music.text and '404' in music.text:
...
def get_musicurl_by_songid(songid):
...
def get_lyric_by_songid(songid):  # /weapi/song/lyric
...
def get_comments_by_songid(songid):  # "pageSize":"1000"--js default --> 10
...
def get_playlists_by_userid(userid):
...
def get_songs_by_playlistid(playlistid):
...
def get_song_detail_by_id(songid):
    ...
    try:
        all_data = response  # json.loads(response.text)
...
def cloud_search_keyword(keyword):  # 测试搜索最多一次100首
...
def get_playlists_by_uid(playlists_params: GetPlaylistsParams):
    user_id = playlists_params.searchText  # 前端请求的参数
    playlist = get_playlists_by_userid(user_id)  # 测试获取用户歌单
    # print(playlist)
...
def get_song_detail_by_sid(songdetail_params: GetSongDetailParams):
    song_id = songdetail_params.song_searchDetail
    playlist = get_song_detail_by_id(song_id)  # 测试获取用户歌单
    print(playlist[0])
...
def get_songs_by_pid(songs_params: GetSongsParams):
    song_id = songs_params.playlistid_searchSongs
    playlist = get_songs_by_playlistid(song_id)  # 测试获取用户歌单
    print(playlist)
...
def get_musicurl_by_sid(music_url_params: GetMusicUrlParams):
    song_id = music_url_params.songid_searchSongUrl
    song_info = get_musicurl_by_songid(song_id)  # 测试获取用户歌单
    print(song_info)
...
def get_musiclyric_by_sid(music_lyric_params: GetMisicLyricParams):
    song_id = music_lyric_params.songid_searchSongLyric
    song_info = get_lyric_by_songid(song_id)  # 测试获取用户歌单
    print(song_info)
    value = {'searchResults': song_info}
    return response_with(resp.SUCCESS_200, value=value)
...
def get_musiccomments_by_sid(music_comments_params: GetMusicCommentsParams):
    song_id = music_comments_params.songid_searchSongComments
    song_info = get_comments_by_songid(song_id)  # 测试获取用户歌单
